...
test.describe("Integração Mistic Pay — hardening e contrato financeiro", () => {
  ...
  test("splitUser e comissão ficam no control plane, sem hidratação no tenant", () => {
    ...
    expect(settings).toContain("PLATFORM_ONLY_SETTING_KEYS")
    expect(settings).toContain("A configuração ${key} pertence ao control plane da plataforma.")
    expect(platformSettings).toContain("misticPaySplitUser")
    expect(platformSettings).toContain("misticPayCommissionCents")
    expect(action).not.toContain("misticpay.splitUser")
    expect(page).not.toContain("misticpay.splitUser")
    expect(form).not.toContain("splitUser")
    expect(form).not.toContain("Deixe em branco para manter o destinatário salvo.")
  })
})
