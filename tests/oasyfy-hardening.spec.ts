...
test.describe("Integração Oasy.fy — contrato e hardening", () => {
  ...
  test("exige dados completos do pagador e coleta PII cifrada no fluxo Telegram", () => {
    ...
    const nameMigration = source("lib/db/migrations/0007_customer_payer_name.sql")
    const records = source("lib/queries/records.ts")

    expect(adapter).toContain("normalizedPayerName.length < 3")
    expect(adapter).toContain("pelo menos 3 caracteres")
    expect(adapter).toContain("!normalizedPayerName || !payerEmail || !payerPhone || !payerDocument")
    ...
    expect(schema).toContain('payerName: text("payerName")')
    expect(migration).toContain('ADD COLUMN IF NOT EXISTS "paymentDataState" text')
    expect(nameMigration).toContain('ADD COLUMN IF NOT EXISTS "payerName" text')
    expect(bot).toContain("validatePayerName")
    expect(bot).toContain("automaticPayerNameFromTelegram")
    expect(bot).toContain("last_name?: string")
