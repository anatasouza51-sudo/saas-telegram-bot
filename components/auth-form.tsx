...
export function AuthForm({ mode }: { mode: "sign-in" | "sign-up" }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-3 sm:gap-8">
      <section className="w-full max-w-2xl px-2 text-center sm:px-0">
        <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-fuchsia-300/20 bg-fuchsia-300/10 px-3 py-1.5 font-space text-[10px] font-bold uppercase tracking-[0.2em] text-fuchsia-100/80 backdrop-blur">
        ...
        <div className="mb-3 flex flex-col items-center gap-1.5 sm:mb-6 sm:gap-3">
          <GhostLogo className="h-10 w-10 sm:h-16 sm:w-16" />
          <div>
            <p className="font-space text-xs font-bold uppercase tracking-[0.24em] text-white/45">Central de comando</p>
            <h1 className="motion-safe:animate-bounce-slow motion-reduce:animate-none font-space text-[2rem] leading-none sm:text-4xl font-black tracking-[-0.05em] text-white sm:text-5xl">
              GHOST <span className="text-fuchsia-300 drop-shadow-[0_0_22px_rgba(232,121,249,0.45)]">BOT</span>
            </h1>
          </div>
        </div>
        <p className="mx-auto max-w-2xl text-xs leading-4 text-white/65 sm:text-lg sm:leading-8">
      ...
      <section className="w-full max-w-[440px]">
          <form
            ...
            className="relative overflow-hidden rounded-[2rem] bg-[#160c28]/70 p-3 shadow-[0_24px_90px_-28px_rgba(168,85,247,0.75)] backdrop-blur-2xl sm:p-8"
          >
          ...
          <div className="relative">
            ...
            {error && <div role="alert" className="mt-4 rounded-xl border border-red-300/20 bg-red-400/10 px-4 py-3 text-xs font-medium leading-5 text-red-100">{error}</div>}

            <button
              type="submit"
              disabled={loading}
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl sm:mt-7 bg-gradient-to-r from-fuchsia-300 via-violet-300 to-purple-300 px-4 py-2.5 font-space sm:py-3.5 text-sm font-black uppercase tracking-wider text-[#160c28] shadow-[0_12px_30px_-10px_rgba(232,121,249,0.9)] transition-all hover:brightness-110 hover:shadow-[0_16px_36px_-10px_rgba(232,121,249,0.95)] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <div className="mt-3 text-center text-xs text-white/45 sm:mt-6">
              {isSignUp ? (
        ...
        <p className="mt-1 text-center font-space text-[9px] sm:mt-2 font-bold uppercase tracking-[0.18em] text-white/25">Seu dinheiro vai direto pro gateway — sem retenção.</p>
      </section>
    </div>
  )
}
